...
# My class
rating = TrueSkill(p_draw=1e-3)
rating.add_players(list_players)
for race_id, dfwk in tqdm.tqdm(df.groupby("race_id")):
    rating.update(*dfwk[["id_racecourse_sk", "id_player_sk"]].to_numpy(dtype=object).tolist(), ranks=dfwk["rank"].tolist(), is_boost=True)

# public class
env = trueskill.TrueSkill(draw_probability=1e-3)
dict_rate = {x: env.Rating() for x in list_players}
for race_id, dfwk in tqdm.tqdm(df.groupby("race_id")):
    list_grps    = dfwk[["id_racecourse_sk", "id_player_sk"]].to_numpy(dtype=object).tolist()
    list_ratings = env.rate([[dict_rate[y] for y in x] for x in list_grps], ranks=dfwk["rank"].tolist(), min_delta=1000)
...
print(list_eval1.mean())
print(list_eval2.mean())
